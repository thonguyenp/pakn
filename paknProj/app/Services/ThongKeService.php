...
{
    public function tongQuan($from, $to)
    {
        ...
        $baseQuery = DB::table('phananh')
            ...
            ->whereBetween('phananh.NgayGui', [$from, $to]);

        // Tổng
        $tongPhanAnh = ($baseQuery)->count();

        // Chưa xử lý
        $chuaXuLy = (clone $baseQuery)
            ->where('phananh.IdTrangThaiPhanAnh', 1)
            ->count();

        // Đang xử lý
        $dangXuLy = (clone $baseQuery)
            ->where('phananh.IdTrangThaiPhanAnh', 3)
            ->count();

        // Hoàn thành
        $daHoanThanh = (clone $baseQuery)
            ->where('phananh.IdTrangThaiPhanAnh', 6)
            ->count();
        ...
        COUNT(DISTINCT phananh.IdPhanAnh) as tong_phan_anh,
        ...
        COUNT(DISTINCT CASE
            ...
            (
                (
                    ...
                    DATE_ADD(
                        phananh.NgayGui,
                        INTERVAL CAST(thxlv.SoGioXuLy AS SIGNED) HOUR
                    )
                )
    ...
    ')
            ...
            ->get();
        $theoDonVi = $theoDonVi->map(function ($item) {
