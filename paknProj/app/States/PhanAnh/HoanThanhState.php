...
class HoanThanhState extends BasePhanAnhState
{
    ...
    public function handle($maTheoDoi, $data, $files)
    {
        $phanHoi = DB::transaction(function () use ($maTheoDoi, $data) {
            ...
            // validate transition
            if (! in_array(6, $this->allowedTransitions($phanAnh->IdTrangThaiPhanAnh))) {
                throw new \Exception('Không thể chuyển sang trạng thái hoàn thành');
            }
        ...
        $this->thongBaoService->create([
            'TieuDe' => 'Đã hoàn thành phản hồi',
            'NoiDung' => 'Đã có phản hồi chính thức cho phản ánh: '.$phanAnh->MaTheoDoi,
            'IdNguoiDung' => $phanAnh->IdNguoiDung,
            'Loai' => 'PHAN_ANH_HOAN_THANH',
            'Link' => [
